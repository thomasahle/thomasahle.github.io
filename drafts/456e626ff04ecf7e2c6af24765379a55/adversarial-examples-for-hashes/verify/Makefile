# verify/Makefile -- build the 23 reproduction programs and check their output.
#
...
DIRS ?= a5hash cityhash-64 farmhash-64 gxhash-64 highwayhash komihash murmurhash3-128 museair rust-ahash spookyhash2-64 t1ha2-64 pengyhash nmhash32 nmhash32x mx3 mir fasthash mum rapidhash-v3 wyhash rapidhash-v1 xxh3-64 xxh3-128
...
# gxhash and aHash have a hardware-AES path that the source enables when the compiler
# advertises the target feature.  Both programs also build and validate without these flags
...
check: all
	...
	for d in $(DIRS); do \
	  case $$d in \
	    ...
	    pengyhash|nmhash32|nmhash32x|mx3|mir|fasthash|mum|rapidhash-v3|wyhash|rapidhash-v1|xxh3-64|xxh3-128) key='collisions = [0-9]+' ;; \
