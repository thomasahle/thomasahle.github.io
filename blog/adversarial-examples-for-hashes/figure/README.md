...
Keep the four marker explanations together above the plot: solid circles,
diamonds, hollow circles, and red crosses. Explain the sampled-rate asterisk
in the article's expandable reading notes, not in the chart legend. The
legend uses one row on desktop and tablet layouts, with shorter labels on
tablets, and wraps into one column on phones to keep the text readable.
Only key assumptions and benchmark caveats belong below the plot. Preserve
the plot's usable height when making room for the legend.
...
Each profile's `smhasher` map supplies a report link for every variant ID.
Use the matching registration and a pinned upstream report, linking to its
summary where available. Reports for custom registrations live in the public
benchmark-reproduction or ChainHash repositories. Mark basic-check-only logs
with `scope: "sanity"` and label them “SMHasher3 basic checks”; these must not be
presented as a full-suite result. These variant-specific links are separate from
the profile's one-to-four general background links.
