...
Finally, note that the change of variables gives $x'(z) = \dfrac{dx}{dz} = z\bigl(1+1/x\bigr)$. From $\log\bigl(z(1+1/x)\bigr) \le 2z/3$ it follows that $z\bigl(1+1/x\bigr) \le e^{2z/3}$ for all $z$. Plugging this into the integral representation
\[
 n! = \left(\tfrac{n}{e}\right)^n \; n \int_{-\infty}^{\infty} e^{-n z^2/2}\, \bigl(z+z/x\bigr)\, dz
\]
we obtain
\[
 n! \le \left(\tfrac{n}{e}\right)^n \; n \int_{-\infty}^{\infty} e^{-n z^2/2 + 2z/3}\, dz
 = \left(\tfrac{n}{e}\right)^n \sqrt{2\pi n}\, e^{2/(9n)},
\]
where the last equality uses completion of the square. This yields the sharpened upper bound $n! \le (n/e)^n\, \sqrt{2\pi n}\, e^{2/(9n)}$.
...
\bibitem{mo-stirling}
T. D. Ahle, ``Simple Stirling Upper/Lower Bounds,'' MathOverflow answer, \url{https://mathoverflow.net/a/484270/5429}.
\bibitem{robbins1955}
H. Robbins, ``A remark on Stirling's formula,'' \emph{The American Mathematical Monthly}, 62(1):26--29, 1955. Available at \url{https://heuklyd.github.io/papers/pdf/Robbins-1955.pdf}.
\end{thebibliography}
