# ChainHash-x86: 24.84 B/cycle on the Xeon 8375C

**Target achieved.** The two final SMHasher3 Speed passes measured **24.84 and
24.78 B/TSC cycle**, versus **19.92** for XXH3-64 and **19.39** for
HalftimeHash-512, taking each hash's best pass. ChainHash-x86 is **24.7% faster
than XXH3-64 and 28.1% faster than HalftimeHash-512**. Its small-key average is
...
|---|---:|---:|---:|---:|---:|
| **chainhash-x86** | **24.84 / 24.78** | **24.84** | 104.34 / 104.36 | **104.34** | — |
| komihash | 7.35 / 7.35 | 7.35 | 26.74 / 26.72 | 26.72 | 1.0000x |
| rapidhash | 10.67 / 10.68 | 10.68 | 27.64 / 27.50 | 27.50 | 1.0009x |
| XXH3-64 | 19.71 / 19.92 | 19.92 | 29.44 / 29.42 | 29.42 | 1.0117x |
| HalftimeHash-512 | 19.39 / 19.32 | 19.39 | 83.10 / 83.22 | 83.10 | 1.0052x |
...
All four bulk controls are within 1.2% of their previous values. Small-key
control ratios are approximately 0.9720, 0.9971, 0.9755 and 0.9692 in table order:
the small-key measurements shifted more than bulk. The new hash's bulk passes
differ by 0.24%. Compared with the previous optimized ChainHash-256 (15.40) and
ChainHash-1k (16.36), the selected result is 1.6130x and 1.5183x respectively,
...
B/TSC. The exploratory Speed run, excluded from final best-of-two selection,
measured 24.82 B/TSC and 104.35 small-key cycles. The implementation was frozen
before the final passes; only the registration's verification constants changed.
...
`bash experiments/verify.sh` for verification. Re-collect existing evidence
without a benchmark using `python3 experiments/collect_results.py`. The speed
runner resumes only matching completed runs; choose a fresh `--out` directory
for a new independent two-pass run.
...
`evidence/smhasher/execution.json` records all ten Speed executions and the
binary hash before/after. Raw Sanity, differential, sanitizer, instruction,
candidate and block-tuning outputs are retained under [evidence](evidence).
