...
nice -n 10 taskset -c 72-79 lake env lean Audit.lean > Audit.txt
nice -n 10 taskset -c 72-79 lake env lean FullAudit.lean > FullAudit.txt
python3 - <<'PY'
from pathlib import Path
import re
for p in Path('ProvenHashes').rglob('*.lean'):
    s=p.read_text()
