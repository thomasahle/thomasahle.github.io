...
assert res[-1] == 2
import json
certificate=json.loads(Path('../reference/irreducibility.json').read_text())
assert int(certificate['modulus'],16)==P
assert [int(x,16) for x in certificate['successive_frobenius_squares']]==res[1:]
...
g, a, b = xgcd(P, res[64] ^ 2)
assert g == 1
assert a == int(certificate["bezout_b"], 16)
assert b == int(certificate["bezout_a"], 16)
...
out += f'\ntheorem residue_bezout : modulus * ({poly(a)}) + (residue_64 - X) * ({poly(b)}) = 1 := by\n'
out += '  unfold modulus residue_64\n  rw [CharTwo.sub_eq_add]\n' + normalizer
out += '\nend ProvenHashes.ChainHash\n'
