...
theorem clnh_natDegree_le {I : Type*} [DecidableEq I]
  ...
  intro i hi
  have h₁ := Polynomial.ofFn_natDegree_lt (R := ZMod 2) (by omega : 1 ≤ 128)
    (m (i, false) + k (i, false))
  have h₂ := Polynomial.ofFn_natDegree_lt (R := ZMod 2) (by omega : 1 ≤ 128)
    (m (i, true) + k (i, true))
  apply Polynomial.natDegree_mul_le.trans
  change (Polynomial.ofFn 128 (m (i, false) + k (i, false))).natDegree +
    (Polynomial.ofFn 128 (m (i, true) + k (i, true))).natDegree ≤ 254
  omega
