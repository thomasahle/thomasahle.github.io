...
(() => {
  ...
  function inspect(row, trigger) {
    ...
    if (row.smhasher.scope === 'sanity') {
      detail.querySelector('.inspector-benchmark-note').textContent += ' The linked SMHasher3 record contains basic sanity checks on Xeon, not a full test-suite result.';
    }
    const links = detail.querySelector('[aria-label="Hash project and background"]');
    links.replaceChildren(...[...profile.links, row.smhasher].map(({label, url}) => {
