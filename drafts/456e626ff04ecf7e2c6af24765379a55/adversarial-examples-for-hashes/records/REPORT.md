...
|---|---|---|---|
| M2Pro | `<repos>/smhasher3/build-speed` | Apple clang version 17.0.0 (clang-1700.6.4.2) | `33670275b735a33de9384d74d970cac932e1d5019ab505f7564c42bc1532b541` |
...
M2 build: `nice -n 10 cmake -S <repos>/smhasher3 -B <repos>/smhasher3/build-speed -DCMAKE_BUILD_TYPE=Release -DENDIAN_DETECT_BUILDTIME=OFF "-DCMAKE_CXX_FLAGS=-Xclang -target-feature -Xclang +aes"`, followed by `nice -n 10 cmake --build <repos>/smhasher3/build-speed --target SMHasher3 -j8`. This was a new build directory. The first configure attempt encountered a missing `Modules/TestEndianess.c.in` in CMake 4.1.2; the successful configuration uses the existing M2 build’s runtime-endianness setting and explicit AES target flag, without changing source. Both attempts and the build log are retained as `provenance/M2Pro-cmake-*.log`.
