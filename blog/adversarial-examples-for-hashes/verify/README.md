...
29 collision-reproduction directories, one per hash or separately tested output/version, plus the separate HalftimeHash audit directory.  Each holds a primary C11 program, a README and (for some)
...
explicit colliding seeds with both hash values.  The two foldhash packages and MuseAir v2 use POSIX threads; the documented smoke commands use one worker. Programs need a C11 compiler, often the GCC/Clang 128-bit integer extension, and libm or pthreads as documented. The sampling programs write their results to stdout/stderr.
...
| `rust-ahash` | aHash 0.8.12 AES | E: 424 B; L=53 (historical A_2: 56 B) | Two-S-box lane echo on the >64-byte path: one AES lane cancels a byte difference twice, six blocks apart, and the shuffled-addition lane cancels the residue; independent key words | 967,503/2^32 ≈ 2^-12.116; cap 17.84 bits | `cd rust-ahash && ./ahash_pairs 24 1 A_2` |
...
| `rapidhash-v3` | rapidhash v3, rapidhash v3 micro, rapidhash v3 nano | 32/32 B, 48/48 B | Paper pairs A (32 B) and D (48 B), in standard, micro and nano v3. Key models: `default` (seed only, shipped eight-word secret) and `random-secret` (seed + eight uniform secret words, the scored model). | Scored (random secret): 2386 / 2^37.83 = 2^-26.61, cap 28.6 bits; see sub-README; 2^20 smoke check is separate from the scored rate | `cd rapidhash-v3 && ./rapidhash_v3_verify 20 [rng seed] [default\|random-secret]` |
| `wyhash` | wyhash final v4.3 | A (32 B) | First-product XOR-fold differential; the rate does not depend on the secret value. Key models: `default` (seed only, shipped secret) and `random-secret` (seed + four uniform secret words, the scored model). | Scored (random secret): 1088 / 3·2^35 = 2^-26.50, cap 28.5 bits; historical default-secret 9 / 2^30; 2^20 is a smoke run only | `cd wyhash && ./wyhash_verify [log2 N] [rng seed] [default\|random-secret]` |
| `rapidhash-v1` | rapidhash v1.0 | A (32 B) | First-product XOR-fold differential; the rate does not depend on the secret value. Key models: `default` (seed only, compiled-in secret) and `random-secret` (seed + three uniform secret words, the scored model). | Scored (random secret): 160 / 2^34 = 2^-26.68, cap 28.7 bits; historical default-secret 12 / 2^30; 2^20 is a smoke run only | `cd rapidhash-v1 && ./rapidhash_v1_verify [log2 N] [rng seed] [default\|random-secret]` |
| `xxh3-64` | XXH3-64 0.8.3 | scored 24 B (W0, L=3); default-secret caveat 32 B (NAF, L=4) | Scored: one keyed word complemented, `fold(x,y) = fold(~x,y)`, through `XXH3_64bits_withSecret` with a fresh 192-byte secret (`random-secret` mode). Caveat: block-1 complement with NAF carry cancellation, needs the default-secret words (`withSeed`, and `withSecretandSeed`/streaming at ≤ 240 B). | Scored (random secret): 527 / 2^36 ≈ 2^-26.96, cap 28.5 bits; default-secret caveat 2,264,081/(3·2^30) ≈ 2^-10.4745, 12.47 bits | `cd xxh3-64 && ./xxh3_64_pair_check 20` (default secret) and `./xxh3_64_pair_check random-secret 20` |
...
| `xxh3-128` | XXH3-128, xxHash 0.8.3 | F (32 B) | Complementary first-word swap; equality of both output halves; the rate does not depend on the secret value. Key models: `default` (seed only, default secret) and `random-secret` (fresh 192-byte secret through `XXH3_128bits_withSecret`, the scored model). | Scored (random secret): 479 / 2^35.46 = 2^-26.56, cap 28.6 bits (full 128-bit collisions); historical default-secret 5 / 2^30 and 165 / 2^34; 2^20 is a smoke run only | `cd xxh3-128 && ./xxh3_128_verify [log2 N] [rng seed] [default\|random-secret]` |
...
Parenthesized timings for the original eleven programs describe the earlier M2 Pro runs. Sample sizes, argument order and threading differ between packages; use the per-hash README for the current command. Some programs check supporting pairs as well as, or instead of, the selected chart pair. The per-hash README identifies that coverage.

## Building and checking

    make            # builds the 27 primary programs listed in the Makefile
    make check      # runs those 27 programs with the commands in the table below
                    # and compares their deterministic counts
...
The Makefile compiles with `cc -O2 -std=c11 … -lm`, adding `-pthread` for MuseAir v2 and `-march=armv8-a+crypto` (arm64) or
`-maes` (x86-64) for the three programs with a hardware-AES path (`gxhash-64`, `rust-ahash`, `go-maphash`);
all three also build and validate without those flags.  `CC`, `CFLAGS` and `LDLIBS` can be
overridden on the command line.  The reference commands use one worker; use `make -j2` to limit parallel compilation to two jobs and `make -j1 check` to run the checks serially.

The two foldhash packages are **not Makefile targets**. Build and run them separately using [fast](foldhash-fast/README.md) and [quality](foldhash-quality/README.md). The [HalftimeHash C++ audit](halftime/README.md) has its own build and execution instructions. The [19 September smoke log](../records/verifier-smoke-2026-09-19.log) records all 27 Makefile checks and both foldhash checks.
...
* **Validation before anything else.**  For registered variants, the SMHasher3 verification value is recomputed with the `lib/Hashinfo.cpp` `_ComputedVerifyImpl`
  ...
  XXH3 uses canonical big-endian output encoding). Packages without that registration use reference vectors or upstream-oracle outputs. Rapidhash-v1 checks the
...
* **Deterministic sampling.**  The documented smoke commands use fixed RNG seeds. The generators differ between packages, and thread counts can change the streams. In particular, foldhash-quality needs an explicit RNG seed for reproducibility; its omitted-seed default uses the time.
...
* **Commands differ by package.** Use the documented argument order and ranges; not every program validates malformed arguments in the same way.
...
* The other hashes are re-implemented from their references and validated against them.  Where
  ...
  MurmurHash3 and SpookyHash are public domain; the historical MuseAir v0.3 code is CC0 1.0. MuseAir v2’s upstream crate is MIT OR Apache-2.0; its C port retains its own MIT notice.
...
The current smoke checks are recorded in [the 19 September log](../records/verifier-smoke-2026-09-19.log).

## Scope of the polished package

All 27 Makefile targets reproduce their deterministic README counts; both separately built foldhash programs pass their reference checks and documented smoke runs. Matching
...
Page scores use L = ceil(max(byte lengths)/8), with L at least one. Most rows allow unequal lengths; the shipped HalftimeHash24 headline concerns its explicitly stated equal-length domain. Exact caps round upward; empirical estimates round to nearest.

Historical default-secret check (not the selected 24-byte random-secret score): [XXH3-64 at 32 bytes](xxh3-64/CHECK_32B.md), program [xxh3_64_32B_check.c](xxh3-64/xxh3_64_32B_check.c); [HalftimeHash independent execution harness](halftime/README.md).
