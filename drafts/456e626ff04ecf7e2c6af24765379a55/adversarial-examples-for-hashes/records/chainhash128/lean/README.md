# Seeded ChainHash model A

The model A additions are `ProvenHashes/SeededPH.lean`,
`ProvenHashes/ModelAStream.lean`, and `ProvenHashes/ModelA.lean`.
The main statements are `ModelA.collision_bound_fixed` and
`ModelA.collision_bound_atMost`, with matching theorems for
`referenceHash (ModelA.expandedKey k)`.

The probability is over ten independent uniform field words. Only the first
32 expanded words are dependent: slot `j` contains `s^(j+1)`. The fixed-length
bound is `min 1 ((d(L) + ceil(L/32) + 1) / 2^64)`, including the one- and
two-word refinements. The separate envelope for arbitrary byte lengths at
...
The local mirror contains no dependency or binary cache, and no local Lean
build is needed.

The remainder describes the inherited ideal-key proof and its source modules.

## Inherited concrete ChainHash proof
