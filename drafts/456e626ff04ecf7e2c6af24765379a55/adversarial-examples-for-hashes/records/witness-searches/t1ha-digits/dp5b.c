// dp5b.c -- refined cost model (digits under the fixed low run of L are free); exhaustive signed-digit route search for t1ha2_atonce (64-bit) fixed pairs
...
// ---- carry-pattern DP with sign accounting ----
// state (C_i, Cp_i, flag): flag = 1 while E_0..E_{i-1} are all 1 (L mod 2^i fixed by the class,
// hence a mod 2^i fixed given t).  A digit at position i costs its sign bit unless flag && E_i.
// Returns total cost (wt(E) + paid sign bits) or -1; fills E and *paid.
static int dp(uint64_t D, uint64_t K, uint64_t S, int c64, int maxcost, uint64_t *Eout, int *paid) {
    int cost[8], ncost[8]; unsigned char bp[64][8];
    for (int s = 0; s < 8; s++) cost[s] = 1 << 20; cost[4] = 0;   // C=0, Cp=0, flag=1
    for (int i = 0; i < 64; i++) {
        int Di = (D >> i) & 1, Ki = (K >> i) & 1, Si = (S >> i) & 1;
        for (int s = 0; s < 8; s++) ncost[s] = 1 << 20;
        int mn = 1 << 20;
        for (int s = 0; s < 8; s++) {
            if (cost[s] >= (1 << 20)) continue;
            int C = s & 1, Cp = (s >> 1) & 1, fl = s >> 2; int Ei = C ^ Di; if (Ei != (Cp ^ Ki)) continue;
            int nfl = fl & Ei; int add = Ei + (Si && !(fl && Ei));
            int c1lo = (C == Di) ? Di : 0, c1hi = (C == Di) ? Di : 1;
            int p1lo = (Cp == Ki) ? Ki : 0, p1hi = (Cp == Ki) ? Ki : 1;
            for (int c1 = c1lo; c1 <= c1hi; c1++) { if (i == 63 && c1 != c64) continue;
                for (int p1 = p1lo; p1 <= p1hi; p1++) { int ns = c1 | (p1 << 1) | (nfl << 2); int v = cost[s] + add;
                    if (v < ncost[ns]) { ncost[ns] = v; bp[i][ns] = (unsigned char)s; if (v < mn) mn = v; } } }
        }
        if (mn > maxcost) return -1;
        memcpy(cost, ncost, sizeof cost);
    }
    int best = -1, bs = 0; for (int s = 0; s < 8; s++) if (cost[s] < (1 << 20) && (best < 0 || cost[s] < best)) { best = cost[s]; bs = s; }
    if (best < 0) return -1;
    uint64_t E = 0; int s = bs;
    for (int i = 63; i >= 0; i--) { s = bp[i][s]; int C = s & 1; E |= (uint64_t)(C ^ ((D >> i) & 1)) << i; }
    // paid sign bits: digits at positions >= r, r = length of the initial 1-run of E
    int r = __builtin_ctzll(~E);
    *paid = __builtin_popcountll(S & (r >= 64 ? 0 : (~0ULL << r)));
    *Eout = E; return best;
}
...
typedef struct {
    ...
    uint64_t D, K, E, du, Da, d0, w0, t, w0s, ts; int c64, wrapu, wrapx, cT, dlen, dpcost, fixed, paid, tok, r;
...
static int realize(const route_t *r, uint64_t Da, uint64_t d0, uint64_t du, uint64_t D, uint64_t K, uint64_t E,
    ...
    for (int tries = 0; tries < 40000; tries++) {
        ...
        for (u128 xx = lo; xx <= hi; xx++) {
            ...
            uint64_t w0 = x - (uint64_t)len1, w0s = xs - (uint64_t)len2, dt = du - Da, t = 0, ts = 0;
            // low r bits of t: a = u - t with u mod 2^r fixed by the class (L mod 2^r = Lval mod 2^r)
            int rr = __builtin_ctzll(~E); uint64_t rmask = rr >= 64 ? ~0ULL : ((1ULL << rr) - 1);
            uint64_t Cfull0 = ((E ^ D) >> 1) | ((uint64_t)c64 << 63); uint64_t Lval0 = Cfull0 & E;
            uint64_t ulow = (Lval0 * P1inv) & rmask; uint64_t areq = 0;
            for (int k = 0; k < r->ndig; k++) if (r->pos[k] < rr && r->sgn[k] < 0) areq |= 1ULL << r->pos[k];
            uint64_t treq = (ulow - areq) & rmask;
            uint64_t w1 = len1 < 16 ? ((1ULL << (8 * (len1 - 8))) - 1) : ~0ULL, w2 = len2 < 16 ? ((1ULL << (8 * (len2 - 8))) - 1) : ~0ULL;
            int tok = 0;
            for (int k = 0; k < 256 && !tok; k++) {
                if (len2 < len1) { ts = ((splitmix64(rng) & ~rmask) | ((treq + dt) & rmask)) & w2; t = ts - dt; }
                else { t = ((splitmix64(rng) & ~rmask) | treq) & w1; ts = t + dt; }
                if ((t & rmask) == treq && (ts & ~w2) == 0 && (t & ~w1) == 0) tok = 1;
            }
            if (!tok) {   // fall back to the plain random tail (signs under the run then cost as usual)
                if (len2 < len1) { ts = splitmix64(rng) & w2; t = ts - dt; }
                else { t = splitmix64(rng) & w1; ts = t + dt; }
            }
            o->tok = tok; o->r = rr;
...
static int PART = 0, NPARTS = 1; static int LOW2 = 64, LOW3 = 64, LOW4 = 64;  // only routes whose second-lowest (third-lowest) digit position <= LOW2 (LOW3)
...
static void process_route(const route_t *r, stats_t *st) {
    ...
    uint64_t d0 = Da * P2inv; uint64_t q = (uint64_t)(((u128)d0 * P2) >> 64);
        ...
        for (u128 duu = lo; duu <= hi; duu++) { if (duu >> 64) break; uint64_t du = (uint64_t)duu;
            ...
            int maxdp = (int)floor(BOUND - wcost + 1e-9);
            if (maxdp < 0) continue;
            uint64_t S = 0; for (int k = 0; k < r->ndig; k++) S |= 1ULL << r->pos[k];
            uint64_t D = du * P1;
            int nKv = r->hasT ? 2 : 1;
            for (int kv = 0; kv < nKv; kv++) for (int wrapx = 0; wrapx < 2; wrapx++) for (int dlen = -7; dlen <= 7; dlen++) {
                ...
                uint64_t E; int paid; int cost = dp(D, K, S, c64, maxdp, &E, &paid); st->dpruns++;
                if (cost < 0) continue;
                double tot = cost + wcost; int wtE = __builtin_popcountll(E);
                if (tot > BOUND + 1e-9) continue;
                st->cands++;
                cand_t c; memset(&c, 0, sizeof c);
                c.tot = tot; c.D = D; c.K = K; c.E = E; c.du = du; c.Da = Da; c.d0 = d0; c.c64 = c64; c.wrapu = wrapu; c.wrapx = wrapx; c.cT = kv; c.dlen = dlen; c.dpcost = wtE; c.paid = paid;
                ...
                if (!ok) {
                    ...
                    printf("UNREAL tot=%.2f dp=%d paid=%d ndig=%d digs=%s Da=%016llx du=%016llx c64=%d wrapu=%d d0=%016llx K=%016llx wrapx=%d cT=%d dlen=%d E=%016llx\n",
                        tot, wtE, paid, r->ndig, digs, (unsigned long long)Da, (unsigned long long)du, c64, wrapu, (unsigned long long)d0, (unsigned long long)K, wrapx, kv, dlen, (unsigned long long)E);
                    fflush(stdout); pthread_mutex_unlock(&outmx);
                    continue;
                }
                st->realized++;
                {   // mini-tune: 24 random tails (same dt, same fixed low bits, same lengths) at 2^12 class samples; keep the best
                    uint64_t dt = c.ts - c.t; int rr = c.r; uint64_t rmask = rr >= 64 ? ~0ULL : ((1ULL << rr) - 1);
                    uint64_t treq = c.t & rmask; uint64_t w1 = c.len1 < 16 ? ((1ULL << (8 * (c.len1 - 8))) - 1) : ~0ULL, w2 = c.len2 < 16 ? ((1ULL << (8 * (c.len2 - 8))) - 1) : ~0ULL;
                    cand_t bestc = c; condcheck(&bestc, 12, &st->rng); uint64_t besth = bestc.condHits;
                    for (int k = 0; k < 24; k++) {
                        cand_t v = c; uint64_t t, ts;
                        if (c.len2 < c.len1) { ts = ((splitmix64(&st->rng) & ~rmask) | ((treq + dt) & rmask)) & w2; t = ts - dt; }
                        else { t = ((splitmix64(&st->rng) & ~rmask) | treq) & w1; ts = t + dt; }
                        if ((ts & ~w2) || (t & ~w1)) continue;
                        v.t = t; v.ts = ts; memset(v.m1 + 8, 0, 8); memset(v.m2 + 8, 0, 8); memcpy(v.m1 + 8, &t, c.len1 - 8); memcpy(v.m2 + 8, &ts, c.len2 - 8);
                        condcheck(&v, 12, &st->rng);
                        if (v.condHits > besth) { besth = v.condHits; bestc = v; }
                    }
                    c = bestc;
                }
                ...
                pthread_mutex_lock(&outmx);
                printf("REAL tot=%.2f est=%.2f dp=%d paid=%d run=%d tok=%d ndig=%d digs=%s len1=%d len2=%d m1=%s m2=%s Lmask=%016llx Lval=%016llx cond=%llu/%llu Da=%016llx du=%016llx c64=%d wrapu=%d d0=%016llx K=%016llx D=%016llx wrapx=%d cT=%d dlen=%d E=%016llx exseed=%016llx\n",
                    tot, c.est, wtE, paid, c.r, c.tok, r->ndig, digs, c.len1, c.len2, h1, h2, (unsigned long long)c.Lmask, (unsigned long long)c.Lval,
    ...
    if (MODE_NDIG == 3) {
        ...
        if (MODE_NDIG == 4) {
            ...
            for (int p2 = 0; p2 < p1; p2++) for (int p3 = 0; p3 < p2; p3++) for (int p4 = 0; p4 < p3; p4++) for (int sg = 0; sg < 32; sg++) {
                if (p3 > LOW2 || p2 > LOW3 || p1 > LOW4) continue;
...
int main(int argc, char **argv) {
    if (argc < 4) { fprintf(stderr, "usage: dp5b <3T|4|4T|5|5T> <threads> <bound> [part/nparts] [low2max] [low3max] [low4max]\n"); return 1; }
    ...
    if (argc > 5) LOW2 = atoi(argv[5]);
    if (argc > 6) LOW3 = atoi(argv[6]);
    if (argc > 7) LOW4 = atoi(argv[7]);
    if (MODE_NDIG == 3) work_total = 63; else work_total = 63 * 62 / 2;
    fprintf(stderr, "mode ndig=%d T=%d threads=%d bound=%.2f units=%ld part=%d/%d low2<=%d low3<=%d low4<=%d\n", MODE_NDIG, MODE_T, NTHREADS, BOUND, work_total, PART, NPARTS, LOW2, LOW3, LOW4);
